...
//     scene.background = new THREE.Color(0x87CEEB); // Sky blue background from your example
...
//     ambientLight = new THREE.AmbientLight(0xffffff, 0.5); // From your example
//     scene.add(ambientLight);

//     directionalLight = new THREE.DirectionalLight(0xffffff, 0.8); // From your example
...
//     enterButton.textContent = 'Войти в комнату (VR)';
//     enterButton.style.position = 'absolute';
//     enterButton.style.top = '10px';
//     enterButton.style.right = '10px';
...
//             // Исправляем направление вперед/назад
...
//             break;
//         case 'Escape':
//             if (!renderer.xr.isPresenting) resetToOrbitView();
//             break;
//     }
// }
...
//         alert("Произошла ошибка при создании модели.");
//         document.getElementById("house-info").textContent = "";
//     }
// }
...
//         alert("Произошла ошибка при обновлении размеров.");
//     }
// }
...
//         controls.enabled = false;
//         setupVRControllers();
//         resetCameraOrientation(); // Сбрасываем ориентацию при входе в VR
//     });
//     renderer.xr.addEventListener('sessionend', () => {
//         console.log('VR session ended');
//         removeVRControllers();
...
//     if (gamepad1 && gamepad1.axes.length >= 2) {
...
//     if (gamepad2 && gamepad2.axes.length >= 2) {
...
// let scene, camera, renderer, controls, ambientLight, directionalLight, hemisphereLight, composer;
// const raycaster = new THREE.Raycaster();
// const mouse = new THREE.Vector2();
// let selectedObject = null;
// const moveState = { forward: false, backward: false, left: false, right: false };
...
//     if (data.type === "floor" || data.color === "#FFA500") {
//         const floorTexture = textureLoader.load('/static/assets/hardwood2_diffuse.jpg');
//         floorTexture.wrapS = floorTexture.wrapT = THREE.RepeatWrapping;
//         floorTexture.repeat.set(4, 4);
//         material = new THREE.MeshStandardMaterial({
//             map: floorTexture,
...
//     scene = new THREE.Scene();
//     scene.background = new THREE.Color(0xFFFFFF); // White background
//     scene.userData = { roomFloors: {} }; // Store floor sizes for rooms
...
//         // Store floor size for this room
//         if (data.room_id) {
//             scene.userData.roomFloors[data.room_id] = {
//                 width: data.size[0],
//                 depth: data.size[2],
//                 height: 3 // Default room height from updateRoomSize
//             };
//         }
//     } else if (data.type === "wall" && data.room_id && scene.userData.roomFloors[data.room_id]) {
//         const room = scene.userData.roomFloors[data.room_id];
//         material = new THREE.MeshStandardMaterial({
//             color: data.color,
//             roughness: 0.8,
//             metalness: 0.2,
...
//         // Determine wall orientation based on size or position
//         const isXWall = Math.abs(data.size[2]) < Math.abs(data.size[0]); // Thin in z, spans x
//         if (isXWall) {
//             geometry = new THREE.BoxGeometry(room.width, room.height, 0.2);
//         } else {
//             geometry = new THREE.BoxGeometry(0.2, room.height, room.depth);
//         }

//         // Adjust position to align with room edges
//         const mesh = new THREE.Mesh(geometry, material);
//         mesh.position.set(...data.position);
//         if (isXWall) {
//             // North/south walls (z-axis offset)
//             if (Math.abs(data.position.z - (room.depth / 2)) < 0.5) {
//                 mesh.position.z = room.depth / 2; // North
//             } else if (Math.abs(data.position.z - (-room.depth / 2)) < 0.5) {
//                 mesh.position.z = -room.depth / 2; // South
//             }
//         } else {
//             // East/west walls (x-axis offset)
//             if (Math.abs(data.position.x - (room.width / 2)) < 0.5) {
//                 mesh.position.x = room.width / 2; // East
//             } else if (Math.abs(data.position.x - (-room.width / 2)) < 0.5) {
//                 mesh.position.x = -room.width / 2; // West
//             }
//         }
//         mesh.position.y = room.height / 2; // Center vertically
//         mesh.castShadow = true;
//         mesh.receiveShadow = true;
//         mesh.rotation.set(
//             THREE.MathUtils.degToRad(data.rotation[0]),
...
//         }
//     });
//     scene.userData.roomFloors = {}; // Reset floor data
// }
...
//             houseData.rooms.forEach(room => {
//                 createObjectFromData(room.floor); // Process floor first to store size
//                 room.walls.forEach(wall => createObjectFromData(wall));
//                 room.roof.color = "#FFFFFF";
//                 room.roof.opacity = 0.5;
//                 createObjectFromData(room.roof);
